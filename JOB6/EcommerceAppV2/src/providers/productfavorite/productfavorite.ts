...
export class ProductfavoriteProvider {
  ...
  constructor(private databaseProvider: DatabaseProvider, private env: EnvProvider, public http: HttpClient) {
  ...
  public insert(product) {
    return this.databaseProvider.getDB().then(
      (db: SQLiteObject) => {
        ...
        return db.executeSql(sql, params).then((s) => console.log(s + 'Product berhasilditambahkan!'))
        .catch(e => console.error('Product gagal ditambahkan:', e));}).catch((e) => console.error(e));
      }

  public remove(id: number) {return this.databaseProvider.getDB().then((db: SQLiteObject) => {
    let sql = 'DELETE FROM myfavorite WHERE id=?';
    let params = [id];
    return db.executeSql(sql, params).then(() => {
    ...
    public getAll() {
      return this.databaseProvider.getDB().then(
        (db: SQLiteObject) => {
          ...
          let param = [];
          return db.executeSql(sql, param).then((data: any) => {
